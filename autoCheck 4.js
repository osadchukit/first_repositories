...
//     .reduce((total, amount) => total + amount.balance, 0);
// };
...
//     if (this.pizzas.find(pizza => pizza === pizzaName) === pizzaName) {
...
//   // firstArray.forEach(element1 => secondArray.find(element2 => {
//   //   if (element1 === element2) {
//   //     return commonElements.push(element2);
//   //   };
//   // }));
...
// const uniqueGenres =
//   allGenres.filter((elem, i, array) => array.indexOf(elem) === i);
...
//     .filter((frend, id, arr) => arr.indexOf(frend) === id); // У декількох користувачів можуть бути однакові друзі, зроби так, щоб масив, що повертається, не містив повторень.
...
//   (time, { playtime, gamesPlayed }) => time + playtime / gamesPlayed,
//   0
// );
...
// const calculateTotalBalance = users.reduce((total, { balance }) => total + balance, 0);
...
//   .filter((friend, index, arr) => arr.indexOf(friend) === index).length;
...
//     .filter((frend, index, arr) => arr.indexOf(frend) === index).sort();
...
//   users
//     .filter(user => user.gender === gender)
//     .reduce((arr, user) => arr + user.balance, 0);
